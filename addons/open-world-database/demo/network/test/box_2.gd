...
func _ready() -> void:
	...
	$Sync.watch(["game_data", "position"]) #this also populates these values with any received data
	$Sync.set_interval(200) #interval for automatic updates
	
func _host_process(delta: float) -> void:
	position.y = 1 + sin(Time.get_ticks_msec() * 0.001) * 2
	
	if Time.get_ticks_msec() > next_update:
		next_update = Time.get_ticks_msec() + $Sync.interval
